...
def process_statements(body: Block, lines: any, first_run: bool, symbol_table: dict, defined_paragraphs: set, defined_sections: set, defined_paragraphs1: set, module_ops: Block = None) -> ModuleOp:
    for op in lines:
        if not op: continue
    ...
    for i in range(start, len(lines)):
        ...
        if operation.get("ACCEPT"):
            ...
            # PROVERA: Da li je ova sekcija već definisana?
            if sec_name in defined_sections or sec_name in defined_paragraphs:
                # Ispisujemo grešku u formatu koji tvoji testovi verovatno traže
                sys.stderr.write(f"SECTION error: redefinition of '{sec_name}'\n")
                sys.exit(1)
            
            # Ako je nova, dodajemo je u set
            ...
            if para_name in symbol_table or  para_name in defined_sections or para_name in defined_paragraphs1:
